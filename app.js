...
app.put('/restore', (req, res) => {
  // define a handler for restore file version
...
app.post('/track', async (req, res) => {
  ...
  // track file changes
  const processTrack = async function processTrack(
    response,
    bodyTrack,
    fileNameTrack,
    userAddressTrack
    ...
    const processForceSave = async function processForceSave(
      downloadUri,
      body,
      fileName,
      userAddress
    ...
    if (bodyTrack.status === 1) {
      // editing
      if (bodyTrack.actions && bodyTrack.actions[0].type === 0) {
        // finished edit
    ...
    } else if (bodyTrack.status === 2 || bodyTrack.status === 3) {
      // MustSave, Corrupted
      await processSave(bodyTrack.url, bodyTrack, fileNameTrack, userAddressTrack) // save file
      return
    } else if (bodyTrack.status === 6 || bodyTrack.status === 7) {
  ...
  try {
    ...
    if (user.id !== 'uid-0') {
      users.getAllUsers().forEach(userInfo => {
        const u = userInfo
        u.image = userInfo.avatar
          ? `${req.DocManager.getServerUrl()}/images/${userInfo.id}.png`
          : null
        usersInfo.push(u)
      }, usersInfo)
    }
    ...
    if (fileExt) {
      ...
      const redirectPath =
        `${req.DocManager.getServerUrl()}/editor?fileName=` +
    ...
    if (cfgSignatureEnable) {
      app.render('config', argss, (err, html) => {
        ...
        if (err) {
          ...
          argss.dataDocument.token = jwt.sign(argss.dataDocument, cfgSignatureSecret, {
            expiresIn: cfgSignatureSecretExpiresIn
          })
          argss.dataSpreadsheet.token = jwt.sign(argss.dataSpreadsheet, cfgSignatureSecret, {
            expiresIn: cfgSignatureSecretExpiresIn
          })
        }
...
app.post('/historyObj', (req, res) => {
  ...
  if (cfgSignatureEnable) {
    for (let i = 0; i < historyObj.historyData.length; i++) {
      // sign token with given data using signature secret
      historyObj.historyData[i].token = jwt.sign(historyObj.historyData[i], cfgSignatureSecret, {
        expiresIn: cfgSignatureSecretExpiresIn
      })
    }
  }
